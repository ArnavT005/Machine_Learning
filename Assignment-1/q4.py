...
def gaussian_discriminant_analysis(gdaX, gdaY):
    ...
    return X, X_mean, X_std, m, Y, phi, mu_1, mu_0, sigma_1, sigma_0, sigma

gdaX = "q4x.dat"
gdaY = "q4y.dat"

X, X_mean, X_std, m, Y, phi, mu_1, mu_0, sigma_1, sigma_0, sigma = gaussian_discriminant_analysis(gdaX, gdaY)
...
output_0 = plt.scatter(output_0_x1, output_0_x2, marker="^", color="red", label="Canada")
# plot points that have target value 1 (Alaska, square)
output_1 = plt.scatter(output_1_x1, output_1_x2, marker="s", color="green", label="Alaska")
...
C = (mu_0.T @ sigma_inv @ mu_0 - mu_1.T @ sigma_inv @ mu_1) / 2 + math.log(phi / (1 - phi))
...
if b == 0:
    if a == 0:
        ...
        hypothesis = plt.axvline(x=(-C / a), label="Linear boundary, $P(y = 1|x:\Theta) = 0.5$")
else:
    hypothesis = plt.plot(x1_sample, (-C[0, 0] - a * x1_sample) / b, label="Linear boundary, $P(y = 1|x:\Theta) = 0.5$")
...
# label axes
plt.xlabel("Ring diameter (Fresh water, normalized with mean=" + str(X_mean[0]) + ", std=" + str(round(X_std[0], 5)) + ")")
plt.ylabel("Ring diameter (Marine water, normalized with mean=" + str(X_mean[1]) + ", std=" + str(round(X_std[1], 5)) + ")")
